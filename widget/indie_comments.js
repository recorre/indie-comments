(function() {
    ...
    async function getSiteByApiKey(apiKey) {
        ...
        // Buscar da API
        const allSitesRes = await apiCall('/read/sites');
    ...
    async function init() {
        ...
        try {
            ...
            let thread;
            const searchThreadsRes = await apiCall(`/search/threads`, {
                method: 'POST',
                body: JSON.stringify({ site_id: site.id, page_identifier: pageIdentifier })
            });
            ...
            const commentsRes = await apiCall(`/read/comments?thread_id=${thread.id}&visible=true`);
